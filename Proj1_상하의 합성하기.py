...
def faceDetect():
    ...
    while True:
        ...
        for (x, y, w, h) in faces: # 여러 faces 중 (x, y, w, h) 는 하나의 묶음
            ...
            crop_img1 = cv.imread('crop.png')
    
            resize_img = cv.resize(crop_img, dsize=(int(3 * w), int(3 * h)), interpolation=cv.INTER_AREA)
            resize_img1 = cv.resize(crop_img1, dsize=(int(2 * w), int(4 * h)), interpolation=cv.INTER_AREA)
            ...
            model[y+h+(int)(h*0.5) + int(3*h):y+h+(int)(h*0.5) + int(3*h) + int(4*h), x - int(w*0.5):x - int(w*0.5) + int(2*w)] = resize_img1 # crop 이미지를 리사이징한 크기로 model 이미지를 다시 잘라야함
